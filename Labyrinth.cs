...
namespace D14_grottaspel
{
    ...
    public class Room
    {
        ...
        {
            Directions D = direction;
            if (D.north != NoDoor && D.north < 0) D.north = -D.north;
            if (D.south != NoDoor && D.south < 0) D.south = -D.south;
            if (D.east != NoDoor && D.east < 0) D.east = -D.east;
            if (D.west != NoDoor && D.west < 0) D.west = -D.west;
        }
    }
